...
public class YutHorseManager : MonoBehaviour
{
    ...
    public void RefreshHorsesAtPosition(int positionIndex, string[] positionNames)
    {
        ...
        else if (positionIndex >= 0 && positionIndex < boardPositions.Length && boardPositions[positionIndex] != null)
        {
            ...
            // 발판 위의 NavMesh를 우선적으로 찾기
            UnityEngine.AI.NavMeshHit hit;
            Vector3 platformTop = basePos;
            platformTop.y += 2.5f; // 발판 높이 고려
            if (UnityEngine.AI.NavMesh.SamplePosition(platformTop, out hit, 5f, UnityEngine.AI.NavMesh.AllAreas))
            {
                if (hit.position.y > basePos.y + 0.5f)
                {
                    finalPos = hit.position; // 발판 위의 NavMesh 사용
                }
            }
            
            // 모든 말을 같은 위치에 배치 (NavMeshAgent 사용)
            foreach (int hIndex in horsesAtPos)
            {
                if (players[hIndex] != null && players[hIndex].transform != null)
                {
                    // NavMeshAgent를 사용하여 이동 (transform.position 직접 설정 제거)
                    UnityEngine.AI.NavMeshAgent agent = players[hIndex].GetComponent<UnityEngine.AI.NavMeshAgent>();
                    if (agent != null && agent.enabled)
                    {
                        ...
                        if (!agent.isOnNavMesh)
                        {
                            ...
                            if (UnityEngine.AI.NavMesh.SamplePosition(players[hIndex].transform.position, out warpHit, 10f, UnityEngine.AI.NavMesh.AllAreas))
                            {
                                agent.Warp(warpHit.position);
                            }
                            else
                            {
                                // NavMesh를 찾을 수 없으면 직접 위치 설정
                        ...
                        {
                            // 여전히 NavMesh 위에 없으면 직접 위치 설정
                            players[hIndex].transform.position = finalPos;
                        }
                    }
                    else
                    {
                        // NavMeshAgent가 없거나 비활성화된 경우에만 직접 설정
                        players[hIndex].transform.position = finalPos;
                    }
                    ...
                    // EF3 (인덱스 22)는 SetHorseRotationByPosition에서 처리하므로 여기서는 설정하지 않음
                    // (경유 시 F 경로면 135도, E 경로면 225도로 설정되어야 함)
