...
public class YutGameManager : MonoBehaviour
{
    ...
    private bool isCameraInCombatMode = false;
    ...
    public IEnumerator MoveHorseInternal(int horseIndex, int steps)
    {
        ...
        for (int i = 0; i < steps; i++)
        {
            ...
            if (isLastStep)
            {
                ...
                {
                    // 잡는 말과 잡히는 말의 위치 가져오기
                    Vector3 attackerPos = players[horseIndex] != null ? players[horseIndex].transform.position : Vector3.zero;
                    Vector3 victimPos = players[enemyHorsesToCapture[0]] != null ? players[enemyHorsesToCapture[0]].transform.position : Vector3.zero;
                    
                    // 카메라를 전투 시점으로 이동
                    StartCoroutine(MoveCameraToCombatView(attackerPos, victimPos));
                    
                    // 카메라 이동 시간 대기 (1초)
                    yield return new WaitForSeconds(1.0f);
                    
                    // 잡힌 말들의 위치를 먼저 수집 (중복 제거 및 업힌 말들 모두 처리하기 위해)
                    HashSet<int> positionsToProcess = new HashSet<int>();
                    foreach (int enemyHorseIndex in enemyHorsesToCapture)
                    {
                        ...
                        if (enemyPos != -1 && enemyPos != -2)
                        {
                            positionsToProcess.Add(enemyPos);
                        }
                    }
                    
                    // 각 위치에 있는 모든 적의 말들을 찾아서 death 애니메이션 준비
                    List<int> allVictimHorses = new List<int>();
                    foreach (int enemyPos in positionsToProcess)
                    {
                        List<int> enemyTeamHorses = new List<int>();
                        
                        // 이 위치에 있는 첫 번째 적의 말을 찾아서 팀 판단
                        bool enemyIsMage = false;
                        bool foundEnemyTeam = false;
                        for (int k = 0; k < playerPositions.Length; k++)
                        {
                            if (playerPositions[k] == enemyPos && playerPositions[k] != -1 && playerPositions[k] != -2)
                            {
                                bool kIsMage = k < 4;
                                // 적의 말인지 확인 (잡는 말과 다른 팀)
                                bool attackerIsMage = horseIndex < 4;
                                if (kIsMage != attackerIsMage)
                                {
                                    enemyIsMage = kIsMage;
                                    foundEnemyTeam = true;
                                    break;
                        ...
                        // 같은 위치에 있는 모든 적의 말들 찾기 (업힌 말들 모두 포함)
                        for (int k = 0; k < playerPositions.Length; k++)
                        {
                            // 같은 위치에 있고, 대기공간(-1)이나 완주(-2)가 아닌 말들만 포함 (업힌 말들만)
                            if (playerPositions[k] == enemyPos && playerPositions[k] != -1 && playerPositions[k] != -2)
                            {
                                bool kIsMage = k < 4;
                        ...
                        allVictimHorses.AddRange(enemyTeamHorses);
                    }
                    
                    // 잡는 말의 공격 애니메이션과 잡히는 말들의 death 애니메이션을 동시에 실행
                    if (players[horseIndex] != null)
                    {
                    ...
                    // 잡히는 말들의 death 애니메이션 동시 실행
                    foreach (int enemyTeamHorse in allVictimHorses)
                    {
                        if (players[enemyTeamHorse] != null)
                        {
                            // GameObject가 비활성화되어 있으면 활성화
                            if (!players[enemyTeamHorse].gameObject.activeInHierarchy)
                            {
                    ...
                    foreach (int enemyTeamHorse in allVictimHorses)
                    {
                        if (players[enemyTeamHorse] != null)
                        {
                            Animator animator = players[enemyTeamHorse].GetComponent<Animator>();
                            if (animator != null)
                            {
                                AnimatorStateInfo stateInfo = animator.GetCurrentAnimatorStateInfo(0);
                                if (stateInfo.IsName("Death"))
                                {
                                    // ExitTime 0.6875 고려하여 대기 시간 계산
                                    float waitTime = stateInfo.length * 0.6875f;
                    ...
                    foreach (int enemyPos in positionsToProcess)
                    {
                        ...
                        foreach (int enemyTeamHorse in enemyTeamHorses)
                        {
                            ...
                            if (players[enemyTeamHorse] != null)
                            {
                                ...
                                // 대기공간으로 이동할 때 회전을 0도로 설정
                                players[enemyTeamHorse].transform.rotation = Quaternion.Euler(0, 0, 0);
                                players[enemyTeamHorse].gameObject.SetActive(true);
                                ...
                                if (horseManager != null)
                                {
                                    horseManager.UpdateHorseCountUI(enemyTeamHorse, 1);
                                }
                            }
                        }
                        
                        // 모든 말의 Death 애니메이션이 완전히 끝날 때까지 대기
                        bool allDeathAnimationsFinished = false;
                        float timeout = 5f; // 타임아웃 증가
                        float elapsed = 0f;
                        while (!allDeathAnimationsFinished && elapsed < timeout)
                        {
                            allDeathAnimationsFinished = true;
                            foreach (int enemyTeamHorse in enemyTeamHorses)
                            {
                                if (players[enemyTeamHorse] != null)
                                {
                                    ...
                                    {
                                        AnimatorStateInfo stateInfo = animator.GetCurrentAnimatorStateInfo(0);
                                        // Death 애니메이션이 재생 중이거나 normalizedTime이 1.0 미만이면 아직 끝나지 않음
                        ...
                        // 추가 대기 시간 (애니메이션 전환 시간 고려)
                        yield return new WaitForSeconds(0.2f);
                        
                        // 모든 말의 Idle 상태로 전환 (각 말마다 확실하게)
                        foreach (int enemyTeamHorse in enemyTeamHorses)
                        {
                            if (players[enemyTeamHorse] != null)
                            {
                                PlayerController playerController = players[enemyTeamHorse].GetComponent<PlayerController>();
            ...
            // E5, D1, D2 발판 디버깅
            bool isDebugPlatform = (playerPositions[horseIndex] == 24 || playerPositions[horseIndex] == 15 || playerPositions[horseIndex] == 16);
            if (isDebugPlatform)
            {
                Debug.Log($"[{horseName}] 발판 이동 - 발판 인덱스: {playerPositions[horseIndex]} ({positionName}), 원본 위치: {originalPosition}");
            }
            ...
            if (isDebugPlatform)
            {
                Debug.Log($"[{horseName}] 발판 이동 - 조정 위치: {adjustedPosition}, 최종 목표: {targetPosition}");
            }
            ...
            if (players[horseIndex] != null)
            {
                ...
                while (!hasReached)
                {
                    ...
                    // E5, D1, D2 발판 디버깅
                    if (isDebugPlatform && Time.frameCount % 30 == 0) // 0.5초마다 로그 출력
                    {
                        Debug.Log($"[{horseName}] 도착 대기 중 - 현재: {players[horseIndex].transform.position}, 목표: {targetPosition}, 거리: {distance:F2}, arrivalDistance: {arrivalDistance:F2}, HasReachedDestination: {players[horseIndex].HasReachedDestination()}");
                    }
                    ...
                    if (distance < arrivalDistance)
                    {
                        ...
                        if (isDebugPlatform)
                        {
                            Debug.Log($"[{horseName}] 도착 완료! - 거리: {distance:F2} < {arrivalDistance:F2}");
                        }
                        break;
                    }
                ...
                {
                    float distance = Vector3.Distance(players[horseIndex].transform.position, targetPosition);
                    // 거리가 2.0f 이상이면 NavMeshAgent로 다시 이동 시도 (강제 이동 제거)
                    // transform.position 직접 설정을 제거하여 NavMeshAgent가 자연스럽게 발판 위로 올라가도록 함
                    if (distance > 2.0f)
                    {
                        UnityEngine.AI.NavMeshAgent agent = players[horseIndex].GetComponent<UnityEngine.AI.NavMeshAgent>();
                        if (agent != null && agent.enabled)
                        {
                            // 발판 위의 NavMesh를 우선적으로 찾기 (여러 높이에서 검색)
                            UnityEngine.AI.NavMeshHit hit;
                            bool foundPlatformNavMesh = false;
                            
                            // 여러 높이에서 발판 위 NavMesh 검색
                            for (float yOffset = 3f; yOffset >= 1f; yOffset -= 0.5f)
                            {
                                Vector3 searchPos = targetPosition;
                                searchPos.y += yOffset;
                                if (UnityEngine.AI.NavMesh.SamplePosition(searchPos, out hit, 5f, UnityEngine.AI.NavMesh.AllAreas))
                                {
                                    // 발판 위의 NavMesh인지 확인
                                    if (hit.position.y > targetPosition.y + 0.3f)
                                    {
                            ...
                            // 발판 위를 찾지 못하면 더 넓은 범위로 재시도
                            if (!foundPlatformNavMesh)
                            {
                                for (float yOffset = 4f; yOffset >= 1f; yOffset -= 0.5f)
                                {
                                    Vector3 searchPos = targetPosition;
                                    searchPos.y += yOffset;
                                    if (UnityEngine.AI.NavMesh.SamplePosition(searchPos, out hit, 10f, UnityEngine.AI.NavMesh.AllAreas))
                                    {
                                        if (hit.position.y > targetPosition.y + 0.3f)
                                        {
                                            agent.SetDestination(hit.position);
                    ...
                    SetHorseRotationByPosition(horseIndex, playerPositions[horseIndex], isLastStep, isFPath);
                    foreach (int otherHorseIndex in horsesToMoveTogether)
                    {
                        SetHorseRotationByPosition(otherHorseIndex, playerPositions[horseIndex], isLastStep, isFPath);
                    }
    ...
    public IEnumerator MoveHorseBackwardInternal(int horseIndex, int steps)
    {
        ...
        // F 경로 추적 (뒤로 이동 시 현재 위치 기반으로 판단)
        bool isFPath = false;
        int currentPosForFPath = playerPositions[horseIndex];
        if (currentPosForFPath == 25 || currentPosForFPath == 26 || currentPosForFPath == 27 || currentPosForFPath == 28 || currentPosForFPath == 10)
        {
            isFPath = true; // F1, F2, F4, F5, C1은 F 경로
        }
        ...
        for (int i = 0; i < steps; i++)
        {
            ...
            // F 경로 추적 업데이트 (F1, F2, F4, F5를 지나가면 F 경로)
            if (playerPositions[horseIndex] == 25 || playerPositions[horseIndex] == 26 || playerPositions[horseIndex] == 27 || playerPositions[horseIndex] == 28)
            {
                isFPath = true;
            }
            ...
            foreach (int otherHorseIndex in horsesToMoveTogether)
            {
                SetHorseRotationByPosition(otherHorseIndex, playerPositions[otherHorseIndex], isLastStep, isFPath);
            }
            if (isLastStep)
            {
                // 도착한 발판에 적의 말이 있으면 잡기 (같은 팀 말 처리 전에)
            ...
            if (players[horseIndex] != null)
            {
                ...
                // 업힌 말들도 동시에 이동 시작
                List<int> movingHorseIndices = new List<int>();
                List<Vector3> movingTargetPositions = new List<Vector3>();
                
                string horseName = horseIndex < 4 ? $"메이지{horseIndex+1}" : $"기사{horseIndex-3}";
                Debug.Log($"[동시 이동] {horseName} (인덱스: {horseIndex}) 이동 시작 - 업힌 말 개수: {horsesToMoveTogether.Count}");
                
                foreach (int otherHorseIndex in horsesToMoveTogether)
                {
                    if (players[otherHorseIndex] != null && players[otherHorseIndex].gameObject.activeInHierarchy)
                    {
                        // 업힌 말들의 목표 위치 계산
                        Vector3 otherOriginalPosition = boardPositions[playerPositions[otherHorseIndex]].position;
                        Vector3 otherAdjustedPosition = horseManager != null 
                            ? horseManager.CalculateHorsePosition(otherHorseIndex, otherOriginalPosition)
                            : otherOriginalPosition;
                        Vector3 otherValidPosition = GetValidNavMeshPosition(otherAdjustedPosition);
                        
                        // 업힌 말들도 초기화 확인
                        if (!players[otherHorseIndex].IsInitialized())
                        {
                            players[otherHorseIndex].InitializeAgent();
                        }
                        
                        string otherHorseName = otherHorseIndex < 4 ? $"메이지{otherHorseIndex+1}" : $"기사{otherHorseIndex-3}";
                        Debug.Log($"[동시 이동] {otherHorseName} (인덱스: {otherHorseIndex}) 동시 이동 시작 - 목표 위치: {otherValidPosition}");
                        
                        players[otherHorseIndex].MoveToPosition(otherValidPosition);
                        movingHorseIndices.Add(otherHorseIndex);
                        movingTargetPositions.Add(otherValidPosition);
                    }
                }
                
                if (movingHorseIndices.Count > 0)
                {
                    Debug.Log($"[동시 이동] 총 {movingHorseIndices.Count}개의 업힌 말이 {horseName}와 함께 동시에 이동 시작");
                }
                ...
                // 업힌 말들도 모두 도착할 때까지 대기
                while (movingHorseIndices.Count > 0)
                {
                    for (int idx = movingHorseIndices.Count - 1; idx >= 0; idx--)
                    {
                        int movingHorseIdx = movingHorseIndices[idx];
                        if (players[movingHorseIdx] != null)
                        {
                            PlayerController controller = players[movingHorseIdx].GetComponent<PlayerController>();
                            if (controller != null && (controller.HasReachedDestination() || Vector3.Distance(players[movingHorseIdx].transform.position, movingTargetPositions[idx]) < (isLastStep ? 0.3f : 0.5f)))
                            {
                                string arrivedHorseName = movingHorseIdx < 4 ? $"메이지{movingHorseIdx+1}" : $"기사{movingHorseIdx-3}";
                                Debug.Log($"[동시 이동] {arrivedHorseName} (인덱스: {movingHorseIdx}) 도착 완료 - 남은 말: {movingHorseIndices.Count - 1}개");
                                movingHorseIndices.RemoveAt(idx);
                                movingTargetPositions.RemoveAt(idx);
                            }
                        }
                        else
                        {
                    ...
                    if (movingHorseIndices.Count == 0)
                    {
                        Debug.Log($"[동시 이동] 모든 업힌 말들이 도착 완료");
                        break;
                    }
                    yield return null;
                }
                ...
                if (players[horseIndex] != null && players[horseIndex].transform != null)
                {
                    ...
                    // 발판에 도착한 후 rotation 설정 (모든 발판에서)
                    SetHorseRotationByPosition(horseIndex, playerPositions[horseIndex], false, isFPath);
                    foreach (int otherHorseIndex in horsesToMoveTogether)
                    {
                        SetHorseRotationByPosition(otherHorseIndex, playerPositions[horseIndex], false, isFPath);
                    }
    ...
    // 카메라를 전투 시점으로 이동
    private IEnumerator MoveCameraToCombatView(Vector3 attackerPosition, Vector3 victimPosition)
    {
        if (mainCamera == null) yield break;
        
        // 원래 위치 저장 (처음 한 번만)
        if (!isCameraInCombatMode)
        {
            originalCameraPosition = mainCamera.transform.position;
            originalCameraRotation = mainCamera.transform.rotation;
            isCameraInCombatMode = true;
        }
        
        // 두 말의 중간 지점 계산
        Vector3 centerPoint = (attackerPosition + victimPosition) / 2f;
        
        // 카메라를 옆쪽에서 보도록 위치 계산
        // 옆쪽 방향: 두 말을 연결하는 벡터의 수직 방향
        Vector3 direction = (victimPosition - attackerPosition).normalized;
        if (direction.magnitude < 0.1f)
        {
            // 두 말이 너무 가까우면 기본 방향 사용
            direction = Vector3.forward;
        }
        Vector3 sideDirection = Vector3.Cross(direction, Vector3.up).normalized;
        if (sideDirection.magnitude < 0.1f)
        {
            // 수직 방향이 없으면 기본 방향 사용
            sideDirection = Vector3.right;
        }
        
        // 카메라 목표 위치 (옆쪽에서 약간 위로) - 거리와 높이 증가
        Vector3 targetPosition = centerPoint + sideDirection * 12f + Vector3.up * 6f;
        
        // 카메라가 두 말을 바라보도록 회전 계산
        Quaternion targetRotation = Quaternion.LookRotation(centerPoint - targetPosition);
    ...
    {
        if (mainCamera == null || !isCameraInCombatMode) yield break;
        ...
        isCameraInCombatMode = false;
    }
    
    // 경로상 다음 위치 계산 (분기 경로 포함)
    // 발판 인덱스에 따라 말의 rotation 설정
    void SetHorseRotationByPosition(int horseIndex, int positionIndex, bool isFinalDestination = false, bool isFPath = false)
    {
        ...
        // EF3 (인덱스 22)에 정확히 도착해서 멈출 때는 135도, 지나갈 때는 F 경로면 135도, E 경로면 225도
        else if (positionIndex == 22)
        {
            if (isFinalDestination)
            {
                rotationY = 135f; // EF3에 정확히 도착해서 멈출 때
            }
            else
            {
                // F 경로에서 EF3을 지나갈 때는 135도, E 경로에서 지나갈 때는 225도
                rotationY = isFPath ? 135f : 225f;
            }
            shouldSetRotation = true;
        }
        ...
        // F1, F2, F4, F5 (인덱스 25, 26, 27, 28)에 도착하면 Y rotation을 135도로 설정
        else if (positionIndex == 25 || positionIndex == 26 || positionIndex == 27 || positionIndex == 28)
        {
            rotationY = 135f;
            shouldSetRotation = true;
        }
    ...
    {
        // 유효한 인덱스만 필터링 (배열 범위 체크)
        List<int> validIndices = new List<int>();
        foreach (int index in platformIndices)
        {
            if (index >= 0 && index < boardPositions.Length)
            {
                validIndices.Add(index);
            }
            else
            {
                Debug.LogWarning($"ShowSelectablePlatforms: 잘못된 발판 인덱스 {index} 제외 (배열 크기: {boardPositions.Length})");
            }
        }
        
        if (platformManager != null)
        {
            platformManager.ShowSelectablePlatforms(validIndices);
            // selectablePlatformIndices는 platformManager에서 관리하지만, 호환성을 위해 유지
            selectablePlatformIndices = new List<int>(validIndices);
        }
        else
        {
    ...
    public IEnumerator MoveToSelectedPlatformInternal(int horseIndex, int targetPlatformIndex, YutOutcome usedMovement = YutOutcome.Nak, bool forceGoalIn = false)
    {
        ...
        if (isBackDo || (usedMovement == YutOutcome.Do && isSpecialBackDoPosition))
        {
            ...
            // 빽도 이동 애니메이션 처리 - 메인 말과 업힌 말들을 동시에 이동 시작
            List<int> movingHorseIndices = new List<int>();
            List<Vector3> targetPositions = new List<Vector3>();
            
            string backDoHorseName = horseIndex < 4 ? $"메이지{horseIndex+1}" : $"기사{horseIndex-3}";
            Debug.Log($"[동시 이동 - 빽도] {backDoHorseName} (인덱스: {horseIndex}) 빽도 이동 시작 - 업힌 말 개수: {horsesToMoveTogether.Count}");
            
            // B1에서 빽도로 A5로 이동하는 경우 0도 방향으로 회전 설정
            int finalPos = playerPositions[horseIndex];
            if (startPosition == 5 && finalPos == 4)
            {
                // 메인 말 회전 설정
                if (players[horseIndex] != null && players[horseIndex].transform != null)
                {
                    Vector3 currentRotation = players[horseIndex].transform.eulerAngles;
                    players[horseIndex].transform.rotation = Quaternion.Euler(currentRotation.x, 0f, currentRotation.z);
                }
                ...
                foreach (int otherHorseIndex in horsesToMoveTogether)
                {
                    ...
                    {
                        Vector3 currentRotation = players[otherHorseIndex].transform.eulerAngles;
                        players[otherHorseIndex].transform.rotation = Quaternion.Euler(currentRotation.x, 0f, currentRotation.z);
                    }
                }
            ...
            if (players[horseIndex] != null)
            {
                ...
                if (playerController != null)
                {
                    Debug.Log($"[동시 이동 - 빽도] {backDoHorseName} (인덱스: {horseIndex}) 동시 이동 시작 - 목표 위치: {targetPosition}");
                    playerController.MoveToPosition(targetPosition);
                    movingHorseIndices.Add(horseIndex);
            ...
            foreach (int otherHorseIndex in horsesToMoveTogether)
            {
                if (players[otherHorseIndex] != null)
                {
                    ...
                    {
                        string otherHorseName = otherHorseIndex < 4 ? $"메이지{otherHorseIndex+1}" : $"기사{otherHorseIndex-3}";
                        Debug.Log($"[동시 이동 - 빽도] {otherHorseName} (인덱스: {otherHorseIndex}) 동시 이동 시작 - 목표 위치: {targetPos}");
            ...
            if (movingHorseIndices.Count > 0)
            {
                Debug.Log($"[동시 이동 - 빽도] 총 {movingHorseIndices.Count}개의 말이 동시에 이동 시작 (메인 말 1개 + 업힌 말 {horsesToMoveTogether.Count}개)");
            }
            
            // 모든 말이 동시에 도착할 때까지 대기
            while (movingHorseIndices.Count > 0)
            {
                for (int i = movingHorseIndices.Count - 1; i >= 0; i--)
                {
                    int movingHorseIdx = movingHorseIndices[i];
                    if (players[movingHorseIdx] != null)
                    {
                        PlayerController controller = players[movingHorseIdx].GetComponent<PlayerController>();
                        if (controller != null && (controller.HasReachedDestination() || Vector3.Distance(players[movingHorseIdx].transform.position, targetPositions[i]) < 0.3f))
                        {
                            string arrivedHorseName = movingHorseIdx < 4 ? $"메이지{movingHorseIdx+1}" : $"기사{movingHorseIdx-3}";
                            Debug.Log($"[동시 이동 - 빽도] {arrivedHorseName} (인덱스: {movingHorseIdx}) 도착 완료 - 남은 말: {movingHorseIndices.Count - 1}개");
                            movingHorseIndices.RemoveAt(i);
                ...
                if (movingHorseIndices.Count == 0)
                {
                    Debug.Log($"[동시 이동 - 빽도] 모든 말들이 도착 완료");
                    break;
                }
                yield return null;
            }
            ...
            if (enemyHorsesToCapture.Count > 0)
            {
                ...
                foreach (int enemyHorseIndex in enemyHorsesToCapture)
                {
                    ...
                    // 대기공간(-1)이나 완주(-2)에 있는 말은 건너뛰기 (업힌 말이 아님)
                    if (oldPosition == -1 || oldPosition == -2)
                    {
                        continue;
                    }
                    positionsToProcess.Add(oldPosition);
                    allVictimHorses.Add(enemyHorseIndex);
                    
                    // 적의 말과 같은 위치에 있던 같은 팀 말들도 추가 (업힌 말들만)
                ...
                // 잡히는 말들의 death 애니메이션 동시 실행
                foreach (int victimHorseIndex in allVictimHorses)
                {
                    if (players[victimHorseIndex] != null)
                    {
                        // GameObject가 비활성화되어 있으면 활성화
                        if (!players[victimHorseIndex].gameObject.activeInHierarchy)
                        {
                            players[victimHorseIndex].gameObject.SetActive(true);
                        }
                        
                        PlayerController victimController = players[victimHorseIndex].GetComponent<PlayerController>();
                        if (victimController != null)
                        {
                            victimController.PlayDeathAnimation();
                        }
            ...
            foreach (int enemyHorseIndex in enemyHorsesToCapture)
            {
                ...
                // playerPositions는 대기공간(-1)으로 설정 (death 애니메이션은 이미 위에서 실행됨)
                ...
                if (players[enemyHorseIndex] != null)
                {
                    ...
                    UnityEngine.AI.NavMeshAgent agent = players[enemyHorseIndex].GetComponent<UnityEngine.AI.NavMeshAgent>();
                    bool wasEnabled = agent != null && agent.enabled;
                    if (agent != null) agent.enabled = false;
                    ...
                    // 대기공간으로 이동할 때 회전을 0도로 설정
                    players[enemyHorseIndex].transform.rotation = Quaternion.Euler(0, 0, 0);
                    ...
                    // Idle 상태로 전환
                    PlayerController playerController = players[enemyHorseIndex].GetComponent<PlayerController>();
                    if (playerController != null)
                    {
                        playerController.SetIdleState();
                    }
                    ...
                    if (horseManager != null)
                    {
                        horseManager.UpdateHorseCountUI(enemyHorseIndex, 1);
                    }
                }
                ...
                for (int k = 0; k < playerPositions.Length; k++)
                {
                    ...
                    if (k != enemyHorseIndex && playerPositions[k] == oldPosition && playerPositions[k] != -1 && playerPositions[k] != -2)
                    {
                        ...
                        if (kIsMage == enemyIsMage)
                        {
                            ...
                            if (players[k] != null)
                            {
                                Vector3 kInitialPosVector = horseInitialPositions[k];
                                
                                // NavMeshAgent 비활성화 후 위치 설정
                                UnityEngine.AI.NavMeshAgent kAgent = players[k].GetComponent<UnityEngine.AI.NavMeshAgent>();
                                bool kWasEnabled = kAgent != null && kAgent.enabled;
                                if (kAgent != null) kAgent.enabled = false;
                                
                                // 실제 transform.position을 대기공간 좌표로 설정
                                players[k].transform.position = kInitialPosVector;
                                // 대기공간으로 이동할 때 회전을 0도로 설정
                                players[k].transform.rotation = Quaternion.Euler(0, 0, 0);
                                players[k].gameObject.SetActive(true);
                                
                                // 위치 설정 후 다시 활성화하고 Warp로 위치 고정
                                if (kAgent != null && kWasEnabled)
                                {
                                    kAgent.enabled = true;
                                    kAgent.Warp(kInitialPosVector);
                                }
                                
                                // Idle 상태로 전환
                                PlayerController kPlayerController = players[k].GetComponent<PlayerController>();
                                if (kPlayerController != null)
                                {
                                    kPlayerController.SetIdleState();
                                }
                                
                                // 머리 위 UI (x2 등) 제거
                                if (horseManager != null)
                                {
                                    horseManager.UpdateHorseCountUI(k, 1);
                                }
                            }
        ...
        for (int i = 0; i < maxSteps; i++)
        {
            ...
            if (players[horseIndex] != null)
            {
                ...
                if (players[horseIndex] != null && players[horseIndex].transform != null)
                {
                    ...
                    SetHorseRotationByPosition(horseIndex, playerPositions[horseIndex], isCurrentStepFinal, isFPath);
                    foreach (int otherHorseIndex in horsesToMoveTogether)
                    {
                        SetHorseRotationByPosition(otherHorseIndex, playerPositions[horseIndex], isCurrentStepFinal, isFPath);
                    }
