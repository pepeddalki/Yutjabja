...
public class YutGameManager : MonoBehaviour
{
    ...
    public IEnumerator MoveHorseInternal(int horseIndex, int steps)
    {
        ...
        for (int i = 0; i < steps; i++)
        {
            ...
            else if (currentPos == 5 && isLastStep) // B1에 마지막 단계로 도착
            {
                nextPos = 20; // E1로 분기
            }
            else if (currentPos == 22 && isLastStep) // EF3에 마지막 단계로 도착
            {
                nextPos = 27; // F4로 분기
            }
            else
    ...
    public List<int> CalculateAvailablePositions(int currentPosition, int moveSteps)
    {
        ...
        if (currentPos == 5) // B1에서 시작
        {
            ...
            moveSteps--; // 이미 E1로 이동했으므로 남은 이동 횟수 감소
        }
        else if (currentPos == 22) // EF3에서 시작
        {
            currentPos = 27; // F4로 분기
            moveSteps--; // 이미 F4로 이동했으므로 남은 이동 횟수 감소
        }
        ...
        for (int i = 0; i < moveSteps; i++)
        {
            ...
            // 다음 위치 계산
            // 현재 위치가 분기점(B1 또는 EF3)이고 마지막 단계에서 도착한 경우 분기 처리
            int nextPos;
            if (currentPos == 5 && isLastStep) // B1에 마지막 단계로 도착
            {
                nextPos = 20; // E1로 분기
            }
            else if (currentPos == 22 && isLastStep) // EF3에 마지막 단계로 도착
